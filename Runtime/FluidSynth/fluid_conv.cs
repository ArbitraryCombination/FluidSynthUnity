...
namespace MidiPlayerTK
{
    ...
    public static class fluid_conv {
         ...
         */
        private const float FLUID_PEAK_ATTENUATION = 960.0f;

        private const int FLUID_CENTS_HZ_SIZE = 1200;
        private const int FLUID_VEL_CB_SIZE = 128;
        private const int FLUID_CB_AMP_SIZE = 961;
        private const int FLUID_ATTEN_AMP_SIZE = 1441;
        private const int FLUID_PAN_SIZE = 1002;

        private const float M_LN10 = 2.3025850929940456840179914546844f;
        ...
        private const float FLUID_ATTEN_POWER_FACTOR = -531.509f;

        /* conversion tables */
        private static readonly float[] fluid_ct2hz_tab = new float[FLUID_CENTS_HZ_SIZE];
        private static readonly float[] fluid_cb2amp_tab = new float[FLUID_CB_AMP_SIZE];
        private static readonly float[] fluid_atten2amp_tab = new float[FLUID_ATTEN_AMP_SIZE];
        private static readonly float[] fluid_concave_tab = new float[128];
        private static readonly float[] fluid_convex_tab = new float[128];
        private static readonly float[] fluid_pan_tab = new float[FLUID_PAN_SIZE];
        ...
        public static void fluid_conversion_config() {
            ...
            for (int i = 0; i < FLUID_CENTS_HZ_SIZE; i++)
            ...
            //float x = Mathf.Log10(128f / 127f);

            /* There seems to be an error in the specs. The equations are
               implemented according to the pictures on SF2.01 page 73. */

            {
                //NOTE(jp): a and b are just unchanging subexpressions factored out of the loop
                const float a = -200f / FLUID_PEAK_ATTENUATION / M_LN10;
                const float b = 1f / ((FLUID_VEL_CB_SIZE - 1) * (FLUID_VEL_CB_SIZE - 1));
                for (int i = 1; i < FLUID_VEL_CB_SIZE - 1; i++) {
                    float x = a * Mathf.Log(i * i * b);
            ...
            {
                const float x = (float) Math.PI / 2f / (FLUID_PAN_SIZE - 1.0f);
