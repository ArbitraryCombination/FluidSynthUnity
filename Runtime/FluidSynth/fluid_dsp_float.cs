...
namespace FluidSynth {
    ...
    public static class fluid_dsp_float
    {
        ...
        private const int interp_coeff_linear_STEP = 2;
        private static readonly float[] interp_coeff_linear = new float[FLUID_INTERP_MAX * interp_coeff_linear_STEP];

        /* 4th order (cubic) interpolation table (4 coefficients centered on 2nd) */
        private const int interp_coeff_STEP = 4;
        private static readonly float[] interp_coeff = new float[FLUID_INTERP_MAX * interp_coeff_STEP];

        /* 7th order interpolation (7 coefficients centered on 3rd) */
        private const int sinc_table7_STEP = 7;
        private static readonly float[] sinc_table7 = new float[FLUID_INTERP_MAX * sinc_table7_STEP];
        ...
        public static void fluid_dsp_float_config() {
            ...
            for (int i = 0; i < FLUID_INTERP_MAX; i++) {
                float x = i / (float)FLUID_INTERP_MAX;

                interp_coeff_linear[i * interp_coeff_linear_STEP + 0] = 1f - x;
                interp_coeff_linear[i * interp_coeff_linear_STEP + 1] = x;

                interp_coeff[i * interp_coeff_STEP + 0] = x * (-0.5f + x * (1f - 0.5f * x));
                interp_coeff[i * interp_coeff_STEP + 1] = 1f + x * x * (1.5f * x - 2.5f);
                interp_coeff[i * interp_coeff_STEP + 2] = x * (0.5f + x * (2f - 1f * x));
                interp_coeff[i * interp_coeff_STEP + 3] = 0.5f * x * x * (x - 1f);
            }
            ...
            for (int i = 0; i < SINC_INTERP_ORDER; i++)
            {
                ...
                for (int i2 = 0; i2 < FLUID_INTERP_MAX; i2++)
                {
                    ...
                    float i_shifted = i - SINC_INTERP_ORDER / 2f + i2 / (float)FLUID_INTERP_MAX;

                    /* sinc(0) cannot be calculated straightforward (limit needed for 0/0) */
                    float v;
                    if (Mathf.Abs(i_shifted) > 0.000001f) {
                        v = Mathf.Sin(i_shifted * Mathf.PI) / (Mathf.PI * i_shifted);
                        /* Hamming window */
                        v *= 0.5f * (1f + Mathf.Cos(2f * Mathf.PI * i_shifted / SINC_INTERP_ORDER));
                    } else
                        v = 1f;

                    sinc_table7[(FLUID_INTERP_MAX - i2 - 1) * sinc_table7_STEP + i] = v;
                }
            }
        }
        ...
        public static int fluid_dsp_float_interpolate_none(fluid_voice voice) {
            ...
            */
            ulong dsp_phase_incr = (((ulong)voice.phase_incr) << 32) |
            ...
            */
            bool looping = (byte)voice.gens[(int)fluid_gen_type.GEN_SAMPLEMODE].Val == (byte)fluid_loop.FLUID_LOOP_DURING_RELEASE ||
                           (byte)voice.gens[(int)fluid_gen_type.GEN_SAMPLEMODE].Val == (byte)fluid_loop.FLUID_LOOP_UNTIL_RELEASE
                           && voice.volenv_section < fluid_voice_envelope_index.FLUID_VOICE_ENVRELEASE;

            uint end_index = looping ? (uint)voice.loopend - 1 : (uint)voice.end;
            ...
            while (true) {
                ...
                */
                ...
                uint dsp_phase_index = ((uint)((dsp_phase + 0x80000000) >> 32));

                /* interpolate sequence of sample points */
                for (; dsp_i < fluid_voice.FLUID_BUFSIZE && dsp_phase_index <= end_index; dsp_i++) {
                ...
                if (dsp_i >= fluid_voice.FLUID_BUFSIZE) break;
            }
        ...
        public static int fluid_dsp_float_interpolate_linear(fluid_voice voice) {
            ...
            uint end_index = (looping ? (uint)voice.loopend - 1 : (uint)voice.end) - 1;

            /* 2nd interpolation point to use at end of loop or sample */
            float point = looping ? dsp_data[voice.loopstart] : dsp_data[voice.end];
            ...
            while (true)
            {
                ...
                */
                uint dsp_phase_index = (uint)(dsp_phase >> 32);
                ...
                for (; dsp_i < fluid_voice.FLUID_BUFSIZE && dsp_phase_index <= end_index; dsp_i++)
                {
                    ...
                    */
                    ...
                    dsp_buf[dsp_i] = dsp_amp * (interp_coeff_linear[tablerow * interp_coeff_linear_STEP + 0] * dsp_data[dsp_phase_index] + interp_coeff_linear[tablerow * interp_coeff_linear_STEP + 1] * dsp_data[dsp_phase_index + 1]);
                ...
                for (; dsp_phase_index <= end_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = dsp_amp * (interp_coeff_linear[tablerow * interp_coeff_linear_STEP + 0] * dsp_data[dsp_phase_index] + interp_coeff_linear[tablerow * interp_coeff_linear_STEP + 1] * point);
        ...
        public static int fluid_dsp_float_interpolate_4th_order(fluid_voice voice) {
            ...
            uint end_index = (uint)(looping ? voice.loopend - 1 : voice.end) - 2;
            ...
            while (true)
            {
                ...
                for (; dsp_phase_index == start_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    uint tablerow = (((uint)((dsp_phase) & 0xFFFFFFFF)) & FLUID_INTERP_BITS_MASK) >> FLUID_INTERP_BITS_SHIFT;

                    dsp_buf[dsp_i] = dsp_amp * (interp_coeff[tablerow * interp_coeff_STEP + 0] * start_point
                                                + interp_coeff[tablerow * interp_coeff_STEP + 1] * dsp_data[dsp_phase_index]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 2] * dsp_data[dsp_phase_index + 1]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 3] * dsp_data[dsp_phase_index + 2]);
                ...
                for (; dsp_i < fluid_voice.FLUID_BUFSIZE && dsp_phase_index <= end_index; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = dsp_amp * (interp_coeff[tablerow * interp_coeff_STEP + 0] * dsp_data[dsp_phase_index - 1]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 1] * dsp_data[dsp_phase_index]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 2] * dsp_data[dsp_phase_index + 1]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 3] * dsp_data[dsp_phase_index + 2]);
                ...
                for (; dsp_phase_index <= end_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = dsp_amp * (interp_coeff[tablerow * interp_coeff_STEP + 0] * dsp_data[dsp_phase_index - 1]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 1] * dsp_data[dsp_phase_index]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 2] * dsp_data[dsp_phase_index + 1]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 3] * end_point1);
                ...
                for (; dsp_phase_index <= end_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = dsp_amp * (interp_coeff[tablerow * interp_coeff_STEP + 0] * dsp_data[dsp_phase_index - 1]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 1] * dsp_data[dsp_phase_index]
                                                + interp_coeff[tablerow * interp_coeff_STEP + 2] * end_point1
                                                + interp_coeff[tablerow * interp_coeff_STEP + 3] * end_point2);
        ...
        public static int fluid_dsp_float_interpolate_7th_order(fluid_voice voice) {
            ...
            */
            ...
            uint end_index = (uint)(looping ? voice.loopend - 1 : voice.end) - 3;
            ...
            while (true)
            {
                ...
                for (; dsp_phase_index == start_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    * (sinc_table7[tablerow * sinc_table7_STEP + 0] * start_points2
                     + sinc_table7[tablerow * sinc_table7_STEP + 1] * start_points1
                     + sinc_table7[tablerow * sinc_table7_STEP + 2] * start_points0
                     + sinc_table7[tablerow * sinc_table7_STEP + 3] * dsp_data[dsp_phase_index]
                     + sinc_table7[tablerow * sinc_table7_STEP + 4] * dsp_data[dsp_phase_index + 1]
                     + sinc_table7[tablerow * sinc_table7_STEP + 5] * dsp_data[dsp_phase_index + 2]
                     + sinc_table7[tablerow * sinc_table7_STEP + 6] * dsp_data[dsp_phase_index + 3]);
                ...
                for (; dsp_phase_index == start_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = dsp_amp
                    * (sinc_table7[tablerow * sinc_table7_STEP + 0] * start_points1
                     + sinc_table7[tablerow * sinc_table7_STEP + 1] * start_points0
                     + sinc_table7[tablerow * sinc_table7_STEP + 2] * dsp_data[dsp_phase_index - 1]
                     + sinc_table7[tablerow * sinc_table7_STEP + 3] * dsp_data[dsp_phase_index]
                     + sinc_table7[tablerow * sinc_table7_STEP + 4] * dsp_data[dsp_phase_index + 1]
                     + sinc_table7[tablerow * sinc_table7_STEP + 5] * dsp_data[dsp_phase_index + 2]
                     + sinc_table7[tablerow * sinc_table7_STEP + 6] * dsp_data[dsp_phase_index + 3]);
                ...
                for (; dsp_phase_index == start_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = dsp_amp
                    * (sinc_table7[tablerow * sinc_table7_STEP + 0] * start_points0
                     + sinc_table7[tablerow * sinc_table7_STEP + 1] * dsp_data[dsp_phase_index - 2]
                     + sinc_table7[tablerow * sinc_table7_STEP + 2] * dsp_data[dsp_phase_index - 1]
                     + sinc_table7[tablerow * sinc_table7_STEP + 3] * dsp_data[dsp_phase_index]
                     + sinc_table7[tablerow * sinc_table7_STEP + 4] * dsp_data[dsp_phase_index + 1]
                     + sinc_table7[tablerow * sinc_table7_STEP + 5] * dsp_data[dsp_phase_index + 2]
                ...
                for (; dsp_i < fluid_voice.FLUID_BUFSIZE && dsp_phase_index <= end_index; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = dsp_amp
                               * (sinc_table7[tablerow * sinc_table7_STEP + 0] * dsp_data[dsp_phase_index - 3]
                                + sinc_table7[tablerow * sinc_table7_STEP + 1] * dsp_data[dsp_phase_index - 2]
                                + sinc_table7[tablerow * sinc_table7_STEP + 2] * dsp_data[dsp_phase_index - 1]
                                + sinc_table7[tablerow * sinc_table7_STEP + 3] * dsp_data[dsp_phase_index]
                                + sinc_table7[tablerow * sinc_table7_STEP + 4] * dsp_data[dsp_phase_index + 1]
                ...
                for (; dsp_phase_index <= end_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    * (sinc_table7[tablerow * sinc_table7_STEP + 0] * dsp_data[dsp_phase_index - 3]
                     ...
                     + sinc_table7[tablerow * sinc_table7_STEP + 4] * dsp_data[dsp_phase_index + 1]
                     + sinc_table7[tablerow * sinc_table7_STEP + 5] * dsp_data[dsp_phase_index + 2]
                     + sinc_table7[tablerow * sinc_table7_STEP + 6] * end_points0);
                ...
                for (; dsp_phase_index <= end_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    * (sinc_table7[tablerow * sinc_table7_STEP + 0] * dsp_data[dsp_phase_index - 3]
                     ...
                     + sinc_table7[tablerow * sinc_table7_STEP + 3] * dsp_data[dsp_phase_index]
                     + sinc_table7[tablerow * sinc_table7_STEP + 4] * dsp_data[dsp_phase_index + 1]
                     + sinc_table7[tablerow * sinc_table7_STEP + 5] * end_points0
                     + sinc_table7[tablerow * sinc_table7_STEP + 6] * end_points1);
                ...
                for (; dsp_phase_index <= end_index && dsp_i < fluid_voice.FLUID_BUFSIZE; dsp_i++)
                {
                    ...
                    * (sinc_table7[tablerow * sinc_table7_STEP + 0] * dsp_data[dsp_phase_index - 3]
                     ...
                     + sinc_table7[tablerow * sinc_table7_STEP + 2] * dsp_data[dsp_phase_index - 1]
                     + sinc_table7[tablerow * sinc_table7_STEP + 3] * dsp_data[dsp_phase_index]
                     + sinc_table7[tablerow * sinc_table7_STEP + 4] * end_points0
                     + sinc_table7[tablerow * sinc_table7_STEP + 5] * end_points1
                     + sinc_table7[tablerow * sinc_table7_STEP + 6] * end_points2);
