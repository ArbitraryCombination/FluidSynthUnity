...
namespace NVorbis {
	internal class Mapping {
		...
		public void Init(Packet packet, int channels, IFloor[] floors, IResidue[] residues) {
			var submapCount = 1;
			...
			for (var j = 0; j < couplingSteps; j++) {
				...
				if (magnitude == angle || magnitude > channels - 1 || angle > channels - 1) throw new InvalidDataException("Invalid magnitude or angle in mapping header!");

				_couplingAngle[j] = angle;
				_couplingMangitude[j] = magnitude;
			}

			if (0 != packet.ReadBits(2)) throw new InvalidDataException("Reserved bits not 0 in mapping header.");

			var mux = new int[channels];
			if (submapCount > 1)
				for (var c = 0; c < channels; c++) {
					mux[c] = (int) packet.ReadBits(4);
					if (mux[c] > submapCount) throw new InvalidDataException("Invalid channel mux submap index in mapping header!");
				}
			...
			for (var j = 0; j < submapCount; j++) {
				...
				if (floorNum >= floors.Length) throw new InvalidDataException("Invalid floor number in mapping header!");

				var residueNum = (int) packet.ReadBits(8);
				if (residueNum >= residues.Length) throw new InvalidDataException("Invalid residue number in mapping header!");
		...
		public void DecodePacket(Packet packet, int blockSize, float[][] buffer) {
			...
			for (var i = 0; i < _submapFloor.Length; i++) {
				for (var j = 0; j < _channelFloor.Length; j++)
					if (_submapFloor[i] != _channelFloor[j] || _submapResidue[i] != _channelResidue[j]) // the submap doesn't match, so this floor doesn't contribute
			...
			for (var c = 0; c < _channelFloor.Length; c++)
				if (floorData[c].ExecuteChannel) {
					...
					Mdct.Reverse(buffer[c], blockSize);
				} else {
