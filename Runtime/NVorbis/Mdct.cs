...
namespace NVorbis {
	...
	internal class Mdct {
		...
		// Static caching is fine, because instances after init are not changed
		// and we only ever lookup a few block sizes that are powers of two
		private static readonly ConcurrentDictionary<int, Mdct> lookupCache = new ConcurrentDictionary<int, Mdct>();

		private static Func<int,Mdct> NewMdct = sc => new Mdct(sc);
		
		public static void Reverse(float[] samples, int sampleCount) {
			var mdct = lookupCache.GetOrAdd(sampleCount, NewMdct);
		...
		internal void CalcReverse(float[] buffer) {
			...
			for (; l < (_ld - 3) >> 1; ++l) {
				...
				var k0_2 = k0 >> 1;
				var lim = 1 << (l + 1);
				for (var i = 0; i < lim; ++i) step3_inner_r_loop(_n >> (l + 4), u, _n2 - 1 - k0 * i, -k0_2, 1 << (l + 3));
			}
