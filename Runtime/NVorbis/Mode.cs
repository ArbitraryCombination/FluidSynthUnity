...
namespace NVorbis {
	internal class Mode {
		...
		public void Init(Packet packet, int channels, int block0Size, int block1Size, Mapping[] mappings) {
			_channels = channels;
			_block0Size = block0Size;
			_block1Size = block1Size;

			_blockFlag = packet.ReadBit();
			if (0 != packet.ReadBits(32)) throw new InvalidDataException("Mode header had invalid window or transform type!");

			var mappingIdx = (int) packet.ReadBits(8);
			if (mappingIdx >= mappings.Length) throw new InvalidDataException("Mode header had invalid mapping index!");
		...
		public bool Decode(Packet packet, float[][] buffer, out int packetStartindex, out int packetValidLength, out int packetTotalLength) {
			if (GetPacketInfo(packet, out var blockSize, out var windowIndex, out _, out packetStartindex, out packetValidLength, out packetTotalLength)) {
				_mapping.DecodePacket(packet, blockSize, buffer);
		...
		private bool GetPacketInfo(Packet packet, out int blockSize, out int windowIndex, out int leftOverlapHalfSize, out int packetStartIndex, out int packetValidLength, out int packetTotalLength) {
