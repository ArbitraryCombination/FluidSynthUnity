...
namespace MidiPlayerTK {
	...
	public class SFLoad {
		...
		private void ProcessInfo(int size) {
			...
			while (size > 0) {
				...
				if (id == File_Chunk_ID.IFIL_ID) {
					...
					if ((id != File_Chunk_ID.ICMT_ID && chunk.size > 256) || (chunk.size > 65536) ||
					    (chunk.size % 2) != 0)
		...
		private void ProcessSDta(int size) {
			...
			if (SFFile.Verbose)
				SFFile.Log(SFFile.LogLevel.Info, "Samplepos:{0} chunk.size:{1}", SfData.samplepos, chunk.size);
		...
		private void LoadPHdr(int size) {
			...
			if (count == 0) { /* at least one preset + term record */
		...
		private void LoadIHdr(int size) {
			...
			for (ii = 0; ii < size; ii++) { /* load all instrument headers */
		...
		private void LoadIBag(int size) {
			...
			if (pz == null) { /* in case that all are no zoners */
					...
					if (z.gens != null) {
						...
						if (!hasInstrument) {
							if (p.GlobalZone != null)
								SFFile.Log(SFFile.LogLevel.Warn,
									"Preset already have a global zone - Preset:{0} ZoneId:{1} Index:{2}", p.Name,
									z.ItemId, z.Index);
							p.GlobalZone = z;
						}
					}
						...
						foreach (HiGen g in z.gens) {
							if (g != null) {
								switch (g.type) {
									...
									case fluid_gen_type.GEN_STARTLOOPADDRCOARSEOFS:
										if (g.Amount != 0) {
											if (z.Index < 0 || z.Index >= sf.Samples.Length ||
											    sf.Samples[z.Index] == null)
												Debug.LogFormat(
													"Instrument:{0} zone:{1} {2} {3} *** error, wave not defined",
													i.Name, z.Index, g.type, g.Amount);
											else {
												sf.Samples[z.Index].Correctedcoarseloopstart = g.Amount;
											}
										}
										g.Val = g.Amount;
										break;
									...
									case fluid_gen_type.GEN_STARTLOOPADDROFS:
										if (g.Amount != 0) {
											if (z.Index < 0 || z.Index >= sf.Samples.Length ||
												...
												sf.Samples[z.Index].Correctedloopstart = g.Amount;
											}
										}
										g.Val = g.Amount;
										break;
									...
									case fluid_gen_type.GEN_ENDLOOPADDRCOARSEOFS:
										if (g.Amount != 0) {
											if (z.Index < 0 || z.Index >= sf.Samples.Length ||
												...
												sf.Samples[z.Index].Correctedcoarseloopend = g.Amount;
											}
										}
										g.Val = g.Amount;
										break;
									...
									case fluid_gen_type.GEN_ENDLOOPADDROFS:
										if (g.Amount != 0) {
											if (z.Index < 0 || z.Index >= sf.Samples.Length ||
												...
												sf.Samples[z.Index].Correctedloopend = g.Amount;
											}
										}
										g.Val = g.Amount;
										break;
		...
		private void FixupSample(SFData sf) {
			foreach (HiSample sam in sf.Samples) {
				...
				if (((sam.SampleType & SFFile.SF_SAMPLETYPE_ROM) != 0 /*&& sam.end > sdtachunk_size*/) ||
				    sam.Start > (sam.End - 4)) {
					SFFile.Log(SFFile.LogLevel.Error,
						"Sample '{0}' start/end file positions are invalid, disabling and will not be saved", sam.Name);
